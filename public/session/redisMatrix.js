...
for (var i = 0, len = nodes.length; i < len; i++) {
    var n = nodes[i];
    nodes[i] = new node({address: n.address, port: n.port,password:n.password});
}
...
module.exports.lpushRedis = function (id, key, value,callback) {
    var client = hashingRing.openClient(id);
    client.lpush(id, key, value,function (err, reply) {
        if (err)
